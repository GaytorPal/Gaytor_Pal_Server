...
module.exports = {
  Query: {
    ...
    async getEventsByDue(_, { target_username, target_dueDate }) {
      ...
      try {
        ...
        const due_events = await Club.aggregate([
          ...
          {
            $match: {
              "events.dueDate": target_dueDate,
              username: target_username,
            },
          },
          { $project: { title: "$events.title", dueDate: "$events.dueDate" } },
        ]);
    ...
    getClubsWithNames: async () => {
      // Find clubs that have a 'club_name' that is not null and not an empty string
      return await Club.find({ club_name: { $nin: [null, ""] } }).select(
        "username club_name description category"
      );
    },
  },
  ...
  Mutation: {
    ...
    addClub: async (
      _,
      { username, email, password, club_name, description, category }
    ) => {
      const existingClub = await Club.findOne({ username });
      if (existingClub) {
        throw new Error("Club already exists with this username.");
      }
